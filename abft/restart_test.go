...
func testRestart(t *testing.T, weights []pos.Weight, cheatersCount int) {
	...
	for n, e := range ordered {
		if r.Intn(10) == 0 || n%20 == 0 {
			...
			restored := NewIndexedForest(store, prev.input, &adapters.VectorToDagIndexer{vecfc.NewIndex(prev.crit, vecfc.LiteConfig())}, prev.crit, prev.config)
			assertar.NoError(restored.Bootstrap(prev.callback))

			lchs[RESTORED].IndexedForest = restored
		}
...
func compareStates(assertar *assert.Assertions, expected, restored *TestForest) {
	assertar.Equal(
...
func compareBlocks(assertar *assert.Assertions, expected, restored *TestForest) {
